...
void minWidth() {
	...
	int mod = polygon.size();
	int i = 0;
	int j = i+1;
	int j0;
	while (area(polygon[i%mod],polygon[(i+1)%mod],polygon[(j+1)%mod]) > area(polygon[i%mod],polygon[(i+1)%mod],polygon[j%mod])) {
		j++;
		j0 = j;
		while (j!=i0) {
			i++;
			minimum = min(minimum,dist(polygon[i%mod],polygon[j%mod]));
			while (area(polygon[i%mod],polygon[(i+1)%mod],polygon[(j+1)%mod]) > area(polygon[i%mod],polygon[(i+1)%mod],polygon[j%mod])) {
				j++;
				if (fabs(dist(polygon[i%mod],polygon[j%mod])-dist(polygon[j0%mod],polygon[i0%mod])) > EPS) {
					minimum = min(minimum,dist(polygon[i%mod],polygon[j%mod]));
				}
				else return;
			}
			if (fabs(area(polygon[j%mod],polygon[(i+1)%mod],polygon[(j+1)%mod])-area(polygon[i%mod],polygon[(i+1)%mod],polygon[j%mod])) < EPS) {
				if (fabs(dist(polygon[i%mod],polygon[j%mod])-dist(polygon[j0%mod],polygon[i0%mod])) > EPS) {
					minimum = min(minimum,dist(polygon[i%mod],polygon[(j+1)%mod]));
				}
				else {
					minimum = min(minimum,dist(polygon[(i+1)%mod],polygon[j%mod]));
				}
			}
		}
	}
